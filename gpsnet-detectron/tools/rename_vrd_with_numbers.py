...
def process_vrd_split(in_split, out_split):
    ...
    for k, v in tqdm(vrd_anns.items()):
        ...
        for a in v:
            ...
            s_box = a['subject']['bbox']
            if not tuple(s_box) in bbox_set:
                bbox_set.add(tuple(s_box))
            o_box = a['object']['bbox']
            if not tuple(o_box) in bbox_set:
                bbox_set.add(tuple(o_box))

        if in_split != 'train' or len(bbox_set) > 1:
            new_anns[new_k] = v
        else:
            print(bbox_set)
