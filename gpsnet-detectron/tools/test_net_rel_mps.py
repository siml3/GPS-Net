...
from core.test_engine_rel_mps import run_inference
...
if __name__ == '__main__':
    ...
    if args.use_gt_boxes:
        if args.use_gt_labels:
            det_file = os.path.join(args.output_dir, 'rel_detections_gt_boxes_prdcls_' + args.load_ckpt.split('_')[-1].split('.')[-2] + '.pkl')
        else:
            det_file = os.path.join(args.output_dir, 'rel_detections_gt_boxes_sgcls_' + args.load_ckpt.split('_')[-1].split('.')[-2] + '.pkl')
    else:
        det_file = os.path.join(args.output_dir, 'rel_detections_' + args.load_ckpt.split('_')[-1].split('.')[-2] + '.pkl')
