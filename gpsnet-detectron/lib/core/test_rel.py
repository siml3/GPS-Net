...
def im_get_det_rels(model, im, dataset_name, target_scale, target_max_size, boxes=None, do_vis=False, roidb=None, use_gt_labels=False):
    ...
    if return_dict['sbj_rois'] is not None:
        ...
        # vaild_boxes = return_dict['vaild_rois'].data.cpu().numpy()[: ,1:5] / im_scale
        # vaild_labels = return_dict['vaild_labels'].data.cpu().numpy() - 1
        if cfg.MODEL.USE_FREQ_BIAS  and 'prd_scores_bias' in return_dict.keys():
            prd_scores_bias = return_dict['prd_scores_bias'].data.cpu().numpy()
        if cfg.MODEL.USE_SPATIAL_FEAT and 'prd_scores_spt' in return_dict.keys():
            prd_scores_spt = return_dict['prd_scores_spt'].data.cpu().numpy()
        if cfg.MODEL.ADD_SCORES_ALL and 'prd_ttl_scores' in return_dict.keys():
        ...
        return_dict2 = dict(sbj_boxes=sbj_boxes,
                            ...
                            # vaild_boxes=vaild_boxes,
                            # vaild_labels=vaild_labels)
        if cfg.MODEL.ADD_SCORES_ALL and 'prd_ttl_scores' in return_dict.keys():
