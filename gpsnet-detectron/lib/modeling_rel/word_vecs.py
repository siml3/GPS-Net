...
def obj_edge_vectors(names, wv_type='glove.6B', wv_dir=cfg.LANGUAGE.WORD_VECS, wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            for t in token.split(' '):
                w = wv_dict.get(t, None)
                if w is not None:
                    wv_index.append(w)
                    string += '+ {} '.format(t)
            if len(wv_index) > 0:
                vectors[i] = sum([wv_arr[j] for j in wv_index]) / len(wv_index)
                print("{} -> ".format(token) + string[1:])
            else:
