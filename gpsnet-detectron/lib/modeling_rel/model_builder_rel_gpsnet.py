...
def get_importance_factor(select_rois, sbj_gt_boxes, obj_gt_boxes, im_info):
    ...
    sbj_count = (box_utils.bbox_overlaps(select_boxes, sbj_gt_boxes) > 0.5).astype(np.float32).sum(-1)
    obj_count = (box_utils.bbox_overlaps(select_boxes, obj_gt_boxes) > 0.5).astype(np.float32).sum(-1)
...
class Generalized_RCNN(nn.Module):
    ...
    def _init_modules(self):
        ...
        if cfg.RESNETS.VRD_PRD_PRETRAINED_WEIGHTS != '' or cfg.VGG16.VRD_PRD_PRETRAINED_WEIGHTS != '' or \
            ...
            self.Box_Head_sg.heads[0].weight.data.copy_(checkpoint['model']['Box_Head.heads.0.weight'])
            self.Box_Head_sg.heads[0].bias.data.copy_(checkpoint['model']['Box_Head.heads.0.bias'])
            self.Box_Head_sg.heads[3].weight.data.copy_(checkpoint['model']['Box_Head.heads.3.weight'])
            self.Box_Head_sg.heads[3].bias.data.copy_(checkpoint['model']['Box_Head.heads.3.bias'])
    ...
    def detector_feature_map(self, blob_conv, det_rois, use_relu=True):
        fpn_ret = {'rois': det_rois}
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
            lvl_min = cfg.FPN.ROI_MIN_LEVEL
            lvl_max = cfg.FPN.ROI_MAX_LEVEL
        ...
        return self.Box_Head(blob_conv, fpn_ret, rois_name='rois', use_relu=use_relu)

    def visual_rep(self, blob_conv_prd, rel_ret, device_id, use_relu=False):
    ...
    def _forward(self, data, im_info, do_vis=False, dataset_name=None, roidb=None, use_gt_labels=False, **rpn_kwargs):
        ...
        # if False:
            ...
            if not self.training:
                ...
                select_twod_inds = np.arange(0,
                                             select_labels_pred.shape[0]) * cfg.MODEL.NUM_CLASSES + select_labels_pred
