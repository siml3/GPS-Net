...
def enumerate_by_image(im_inds_np):
...
class single_scale_pairs_pn_outputs(nn.Module):
    ...
    def forward(self, det_rois, det_labels, det_scores, im_info, dataset_name, roidb=None):
        ...
        if self.overlap:
            is_cand = is_cand & (box_utils.bbox_overlaps(det_rois[:, 1:], det_rois[:, 1:]) > 0)
        if is_empty.size > 0:
...
def rel_samples(det_rois, edge_inds, im_info, roidb):
    ...
    for i, edge_indices in enumerate(edge_indices_sets):
        ...
        sbj_gt_rois_i = roidb[i]['sbj_gt_boxes'] * im_info[i, 2].data.cpu().numpy()
        obj_gt_rois_i = roidb[i]['obj_gt_boxes'] * im_info[i, 2].data.cpu().numpy()
        ...
        edge_rels_i[fg_inds_i, -1] = prd_gt_classes_i[min_ious_i.argmax(-1)[fg_inds_i]]
    ...
    num_bg = min(bg_rels.shape[0],
                int(cfg.TRAIN.FG_REL_SIZE_PER_IM / cfg.TRAIN.FG_REL_FRACTION * num_img - num_fg))
...
def co_nms_wrapper(det_rois, det_scores, prd_dists, edge_inds, nms_thr):
    num_img = int(edge_inds[:, 0].max()) + 1
    edge_indices_sets = [np.where(edge_inds[:, 0] == i)[0] for i in range(num_img)]
    keep_edge_indices = []
    prd_scores = prd_dists[:, 1:].max(-1)[0].data.cpu().numpy()
    pair_scores = np.expand_dims(det_scores[edge_inds[:, 1]] * det_scores[edge_inds[:, 2]] \
                     * prd_scores, -1)
    p_dets = np.concatenate((det_rois[:, 1:][edge_inds[:, 1]], det_rois[:, 1:][edge_inds[:, 2]], \
                pair_scores), -1)

    for i, edge_indices in enumerate(edge_indices_sets):

        keep = box_utils.co_nms(p_dets[edge_indices], nms_thr)
        keep_inds = edge_indices[keep]
        keep_edge_inds = edge_inds[keep_inds]
        topk_inds = np.argsort(-pair_scores[keep_inds].ravel())[:100]
        topk_det_inds = np.unique(keep_edge_inds[topk_inds][:, 1:].ravel())

        keep_inds2 = np.where((topk_det_inds[:, None] == keep_edge_inds[:, 1][None, :]).any(0) & \
                              (topk_det_inds[:, None] == keep_edge_inds[:, 2][None, :]).any(0))[0]
        assert (np.unique(edge_inds[keep_inds][keep_inds2][:, 1:]) == topk_det_inds).all()
        
        keep_edge_indices.append(keep_inds[keep_inds2])
    
    keep_edge_indices = np.concatenate(keep_edge_indices, 0)

    return keep_edge_indices

def topk_batch(edge_ret):
    so_scores = edge_ret['sbj_scores'] * edge_ret['obj_scores']
    num_img = int(edge_ret['sbj_rois'][:, 0].max()) + 1
    edge_indices_sets = [np.where(edge_ret['sbj_rois'][:, 0] == i)[0] for i in range(num_img)]
    keep_edge_indices = []

    for i, edge_indices in enumerate(edge_indices_sets):
        topk_indices = np.argsort(-so_scores[edge_indices])[:1000]
        keep_edge_indices.append(edge_indices[topk_indices])
    
    keep_edge_indices = np.concatenate(keep_edge_indices, 0)
    return keep_edge_indices


def topk_batch_sg(det_rois, det_scores, prd_dists, edge_inds, nms_thr):
    num_img = int(edge_inds[:, 0].max()) + 1
    edge_indices_sets = [np.where(edge_inds[:, 0] == i)[0] for i in range(num_img)]
    keep_edge_indices = []
    prd_scores = prd_dists[:, 1:].max(-1)[0].data.cpu().numpy()
    ...
    for i, edge_indices in enumerate(edge_indices_sets):
        ...
        topk_inds = np.argsort(-pair_scores[edge_indices].ravel())[:100]


        keep_edge_indices.append(edge_indices[topk_inds])
