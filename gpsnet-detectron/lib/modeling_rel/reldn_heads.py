...
class union_mask(nn.Module):
    def __init__(self, dim_in):
        super().__init__()
        self.dim_in = dim_in
        self.conv = nn.Sequential(
            nn.Conv2d(2, dim_in //2, kernel_size=7, stride=2, padding=3, bias=True),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(dim_in//2, momentum=0.01),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            nn.Conv2d(dim_in // 2, dim_in, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(dim_in, momentum=0.01),
        )

    def forward(self, rpn_ret, device_id):
        pair_rois = np.concatenate((rpn_ret['sbj_rois'][:, 1:], rpn_ret['obj_rois'][:, 1:]), -1)
        rects_np = draw_union_boxes(pair_rois, 7*4-1) - 0.5
        rects = torch.from_numpy(rects_np).float().cuda(device_id)
        return self.conv(rects)
...
class roi_2mlp_head_prd(nn.Module):
    ...
    def forward(self, x, rpn_ret, mask, rois_name, use_relu=False):
        x = self.roi_xform(
            x, rpn_ret,
            blob_rois=rois_name,
