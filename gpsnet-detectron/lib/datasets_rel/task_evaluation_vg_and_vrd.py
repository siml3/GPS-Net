...
def eval_rel_results(all_results, output_dir, do_val):
    ...
    for phrdet in eval_sets:
        ...
        for prd_k in prd_k_set:
            ...
            recalls = {20: 0, 50: 0, 100: 0, 200: 0}
            pairs_recalls = {20: 0, 50: 0, 100: 0, 200: 0}
            if do_val:
                all_gt_cnt = 0
            ...
            for im_i, res in enumerate(tqdm(all_results)):
                ...
                if do_val:
                    ...
                    if phrdet:
                        ...
                        det_boxes_pair_top = boxes_union(det_boxes_sbj[det_so_scores_inds],
                                                         det_boxes_obj[det_so_scores_inds])
                    ...
                    for k in recalls:
                        ...
                        if len(pred_to_gt_pair):
                            match_pair = reduce(np.union1d, pred_to_gt_pair[:k])
                        else:
                            match_pair = []

                        pairs_recalls[k] += len(match_pair)
            ...
            if do_val:
                ...
                for k in pairs_recalls:
                    pairs_recalls[k] = float(pairs_recalls[k]) / (float(all_gt_cnt) + 1e-12)
