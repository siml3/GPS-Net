...
def eval_rel_results(all_results, output_dir, do_val):
    ...
    for phrdet in eval_sets:
        ...
        for prd_k in prd_k_set:
            ...
            pairs_recalls = {20: [], 50: [], 100: []}
            vaild_rois_recalls = {3: [], 4: [], 5: [], 6: [], 7: [], 8: [], 9: [], 10: [], 15: [], 20: [], 30:[], 50:[], 100:[], 200:[]}
            vaild_rois_accuracy = {3: [], 4: [], 5: [], 6: [], 7: [], 8: [], 9: [], 10: [], 15: [], 20: [], 30:[], 50:[], 100:[], 200:[]}
            if do_val:
                all_gt_cnt = 0
            ...
            for im_i, res in enumerate(tqdm(all_results)):
                ...
                if res['prd_scores'] is None:
                    ...
                    det_so_scores_inds = np.argsort(-det_scores_so)

                    det_labels_so_top = np.vstack(
                        (det_labels_sbj[det_so_scores_inds], det_labels_obj[det_so_scores_inds])).transpose()
                    det_boxes_so_sort = np.hstack(
                        (det_boxes_sbj[det_so_scores_inds], det_boxes_obj[det_so_scores_inds]))
                ...
                if do_val:
                    ...
                    if gt_boxes_obj.size > 0:
                        unique_boxes, unique_inds = np.unique(np.concatenate((gt_boxes_sbj, gt_boxes_obj), 0), axis=0, return_index=True)
                        unique_labels = np.concatenate((gt_labels_sbj, gt_labels_obj), 0)[unique_inds]
                    else:
                        unique_boxes = np.empty((0, 4), dtype=np.float32)
                        unique_labels = np.empty((0), dtype=np.int64)
                    ...
                    gt_labels_so = np.vstack((gt_labels_sbj, gt_labels_obj)).transpose()
                    ...
                    if phrdet:
                        ...
                        det_boxes_pair_top = boxes_union(det_boxes_sbj[det_so_scores_inds], det_boxes_obj[det_so_scores_inds])
                        gt_boxes_r = boxes_union(gt_boxes_sbj, gt_boxes_obj)
                        pred_to_gt = _compute_pred_matches(
                            gt_labels_spo, det_labels_spo_top,
                            gt_boxes_r, det_boxes_r_top,
                            phrdet=phrdet)
                        pred_to_gt_pair = _compute_pred_matches(
                            gt_labels_so, det_labels_so_top,
                            gt_boxes_r, det_boxes_pair_top,
                            phrdet=phrdet)
                    else:
                        pred_to_gt = _compute_pred_matches(
                            gt_labels_spo, det_labels_spo_top,
                    ...
                    for k in recalls:
                        if len(pred_to_gt):
                            ...
                            match = []
                        recalls[k] += len(match)
                        pairs_recalls[k].append(len(match_pair) / max(gt_labels_so.shape[0], 1e-12))




                        # for k in rois_redun:
                        #     boxes_redun = ious[:k].sum(0)
                        #     vaild_inds = np.where(boxes_redun > 0)[0]
                        #     if vaild_inds.size > 0:
                        #         rois_redun[k].append(boxes_redun[vaild_inds].mean())
            ...
            if do_val:
                ...
                print_stats(recalls)
                print('====================== ' + 'pairs recalls' + ' ============================')
                for k in pairs_recalls:
                    pairs_recalls[k] = np.mean(pairs_recalls[k])
                print_stats(pairs_recalls)
                # print('====================== ' + 'vaild rois recalls' + ' ============================')
                # for k in vaild_rois_recalls:
                #     vaild_rois_recalls[k] = np.mean(vaild_rois_recalls[k])
                # print_stats(vaild_rois_recalls)
                # print('====================== ' + 'vaild rois accuracy' + ' ============================')
                # for k in vaild_rois_accuracy:
                #     vaild_rois_accuracy[k]= np.mean(vaild_rois_accuracy[k])
                # print_stats(vaild_rois_accuracy)
